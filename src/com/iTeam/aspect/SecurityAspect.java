...
/**
 * 拦截Controller类的方法，并从请求头中获取token，最后对token有效性进行判断。 Title:安全检查切面(是否登录检查)
...
//			if (cookies[i].getName().equals(MyConstants.DEFAULT_TOKEN_NAME)) {
//				token = cookies[i].getValue();
//			}
//		}

		// 检查 token 有效性
		log.debug("发起请求：" + WebContextUtil.getRequest().getLocalName() + ";token:" + token);
